function ce01_general, fluxes, flux_errs, redshift, lir_err = ce_lir_err, pred_fluxes = pred_fluxes

; fluxes must have entries in the following order:
; mips 24, mips70, pacs 70, pacs 100, pacs 160, spire 250, spire 350,
; spire 500, alma 870.  Use NaN if the flux and its error are not available.
...
format = '(d,d)', /silent
readcol, 'pacs_100_response.dat', wave_pacs100, $
resp_pacs100, format = '(d,d)', /silent
readcol, 'pacs_160_response.dat', wave_pacs160, $
resp_pacs160, format = '(d,d)', /silent
readcol, 'spire_250_response.dat', wave_spire250, $
resp_spire250, format = '(d,d)', /silent
readcol, 'spire_350_response.dat', wave_spire350, $
resp_spire350, format = '(d,d)', /silent
readcol, 'spire_500_response.dat', wave_spire500, $
resp_spire500, format = '(d,d)', /silent

wave_alma870 = 2.99793d5/(dindgen(161)/20.+341.)
resp_alma870 = dblarr(161)+1.
...
norm_alma870 = 1.d-32*tsum(c_light/wave_alma870, $
                           resp_alma870*c_light/wave_alma870)
...
for i=0,num_models-1 do begin
   ...
   nuSnu_interp_alma870 = interpol(nuSnu[i,*], lambda_obs, wave_alma870)
   Snu_alma870[i] = tsum(c_light/wave_alma870, $
                         resp_alma870*nuSnu_interp_alma870)/norm_alma870


endfor

model_fluxes = [[Snu_mips24], [Snu_mips70], [Snu_pacs70], [Snu_pacs100], $
                [Snu_pacs160], [Snu_spire250], [Snu_spire350], $
                [Snu_spire500], [Snu_alma870]]
